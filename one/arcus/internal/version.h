...
// 18 Jul 2020
// The Animation Compression Library finally supports backward compatibility (going forward once 2.0
// comes out). I’m really happy with how it turned out so I thought I would write a bit about how
// the ACL decompression is designed.
//
// The API
...
// A small context object is created and bound to our compressed data. Its construction is cheap
// enough that it can be done on the stack on demand. It can then subsequently be used (and re-used)
// to seek and decompress.
//
// A key design goal is to have as little overhead as possible: pay only for what you use. This is
// achieved through templating in two ways:
//
// The custom_decompression_settings argument on the context object controls what features are
// enabled or disabled. The custom_writer wraps whatever container you might be using in your game
// engine to represent the animation data. This is to make sure no extra copying is required. The
// decompression settings are where the magic happens.
//
// Compile time user control
// There are many game engines out there each handling animation in their own specific way. In order
// to be able to integrate as seamlessly as possible, ACL exposes a small struct that can be
// overriden to control its behavior. By leveraging constexpr and templating, features that aren’t
// used or needed can be removed entirely at compile time to ensure zero cost at runtime.
//
...
//   // Other stuff ...
// };
// A new struct is created to inherit from the desired decompression settings and specific functions
// are defined to hide the base implementations, thus replacing them.
//
// By templating the decompression_context with the settings structure, it can be used to determine
// everything needed at compile time:
//
// Which memory representation is needed depending on whether we are decompressing scalar or
// transform tracks. Which algorithm version to support and optimize for. Which features to strip
// when they aren’t needed. This is much nicer than the common C way to use #define macros. By using
// a template argument, multiple setting objects can easily be created (with type safety) and used
// within the same application or file.
//
// Backward compatibility
// By using the decompression_settings, we can specify which version we optimize for. If no specific
// version is provided (the default behavior), we will branch and handle all supported versions.
// However, if a specific version is provided, we can strip the code for all other versions removing
// any runtime overhead. This is clean and simple thanks to templates.
//
...
//   {
//     // TODO: Note that the `any` decompression can be optimized further to avoid a complex switch
//     on every call. const compressed_tracks_version16 version = tracks.get_version(); switch
//     (version)
//     {
...
//   // Other stuff ...
// };
// This is ideal for many game engines. For example Unreal Engine 4 always compresses locally and
// caches the result in its Derived Data Cache. This means that the compressed format is always the
// latest one used by the plugin. As such, UE4 only needs to support a single version and it can do
// so without any overhead.
//
// Other game engines might choose to support the latest two versions, emiting a warning to
// recompress old animations while still being able to support them with very little overhead: a
// single branch to pick which context to use.
//
// More general applications might opt to support every version (e.g. a glTF viewer).
//
// Note that backward compatibility will only be supported for official releases as the develop
// branch is constantly subject to change.
//
// Conclusion
// This C++ customization pattern is clean and simple to use and it allows a compact API with a rich
// feature set. It was present in a slightly different form ever since ACL 0.1 and the more I use
// it, the more I love it.
//
// In fact, in my opinion the Animation Compression Library and Realtime Math contain some of the
// best code (quality wise) that I’ve ever written in my career. Free from time or budget
// constraints, I can carefully craft each facet to the best of my ability.
//
