...
protected:
    ...
    void tally_game_error(Error e);

    // Register an error in the agent tally.
    void tally_agent_error(Error e);
...
public:
    ...
    void set_stress_game_callback(
        std::function<Error(one_integration::Game &, int percent)> callback);
    void set_stress_agent_callback(std::function<Error(Agent &, int percent)> callback);

private:
    std::function<Error(one_integration::Game &, int percent)> _stress_game_callback;
    std::function<Error(Agent &, int percent)> _stress_agent_callback;
};
...
public:
    ...
    void set_random_game_callback(
        std::function<Error(one_integration::Game &, int percent)> callback);
    void set_random_agent_callback(std::function<Error(Agent &, int percent)> callback);

private:
    std::function<Error(one_integration::Game &, int percent)> _random_game_callback;
    std::function<Error(Agent &, int percent)> _random_agent_callback;
};
